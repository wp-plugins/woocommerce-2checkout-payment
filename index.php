...
function nm_2co_settins( $links ) {
    $settings_link = '<a href="'.admin_url( 'admin.php?page=wc-settings&tab=checkout&section=wc_gateway_nm_twocheckout' ).'">Settings</a>';
  	array_push( $links, $settings_link );
  	return $links;
}
$plugin = plugin_basename( __FILE__ );
add_filter( "plugin_action_links_$plugin", 'nm_2co_settins' );
...
function init_nm_woo_gateway(){
	...
	class WC_Gateway_NM_TwoCheckout extends WC_Payment_Gateway {
		...
		public function __construct(){
			...
			$this->method_description 	= 'WooCommerce 2Checkout Payment Gateway';
		...
		function init_form_fields(){
			...
			$this->form_fields = array(
					'enabled' => array(
							...
							'type' => 'checkbox',
							'label' => __( 'Enable/Disable this gateway', 'woocommerce' ),
							'default' => 'yes'
					),
					'inline' => array(
					...
					'demo' => array(
							...
							'type' => 'checkbox',
							'label' => __( 'Enable Demo Mode', 'woocommerce' ),
							'default' => 'yes'
					),
					'pay_method' => array(
		...
		function process_payment( $order_id ) {
			...
			return array(
					'result' 	=> 'success',
					'redirect'	=> $this -> checkout_url.'?'.$twoco_args
			);
		...
		function twocheckout_response() {
			...
			do_action( "process_2co_ipn_request", $_POST );			
